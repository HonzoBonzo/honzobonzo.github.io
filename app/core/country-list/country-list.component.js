...
function CountryListController(CountryService) {
  ...
  vm.maybeChangeRegionGeoPermission = function (countryRegionName) {
    ...
    vm.regions = [].concat(_toConsumableArray(vm.regions.slice(0, index)), [_extends({}, getRegion(countryRegionName), {
  ...
  vm.changeAllCountries = function (regionName) {
    var changedCountries = vm.countries.filter(function (country) {
  ...
  vm.sortByName = function (regionName) {
    ...
    vm.regions = [].concat(_toConsumableArray(vm.regions.slice(0, index)), [_extends({}, getRegion(regionName), {
